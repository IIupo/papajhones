...
/**
 * Mark a batch of orders as delivered if the courier is within DELIVERY_RADIUS_M of each order.
 * Returns results in the same order as orderIds.
 */
export async function deliverOrdersBatch(courierId: string, orderIds: string[], geo: Geo): Promise<DeliveryResult[]> {
  ...
  try {
    ...
    const selectRes = await client.query(
      `SELECT id, delivery_lat, delivery_lng, assigned_to
       FROM orders
       WHERE id = ANY($1::text[])
